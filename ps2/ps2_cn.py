...
def hangman_with_hints(secret_word):
    ...
    * If the guess is the symbol *, print out all words in wordlist that
    ...
    # FILL IN YOUR CODE HERE AND DELETE "pass"
    print("嗨，我们来玩猜单词的游戏吧！") 
    print("我来想一个单词，你来猜，单词的长度是",len(secret_word),"个字母")
    guesses = 7
    warnings = 3
    letters_guessed = []
    print("你有",warnings,"次接受警告的机会")
    print("你有一次提示的机会，输入*号就可以获得提示，只有一次机会，请谨慎使用哦！\n"+
        "例如(t_ _ t) 提示后可以得到：\n"+
        "tact tart taut teat tent test text that tilt tint toot tort tout trot tuft twit")
    while  not is_word_guessed(secret_word, letters_guessed) and guesses > 0:
        print("--------------------------")
        print("您还有",guesses,"次猜单词的机会")
        print("您可以输入的字母：",get_available_letters(letters_guessed))
        input_user = input("请输入一个字母：")
        if input_user == '*':
            ...
            warnings -= 1
            if warnings > 0:
                print("您输入是非法字符哦！你还有",warnings,"次警告的机会：",
                get_guessed_word(secret_word,letters_guessed))
            else:
                print("您输入是非法字符哦！您已经没有警告的机会了\n" + 
                    "所以扣掉你一次猜单词的机会~~：",get_guessed_word(secret_word,letters_guessed))
                guesses -= 1
            continue
        elif input_user in letters_guessed:
            warnings -= 1
            if warnings > 0:
                print("这个字母您已经输入过一次了！ 你还有",warnings,"次警告的机会：",
                get_guessed_word(secret_word,letters_guessed))
            else:
                print("这个字母您已经输入过一次了！您已经没有警告的机会了\n" + 
                    "所以扣掉你一次猜单词的机会~~：",get_guessed_word(secret_word,letters_guessed))
                guesses -= 1
            continue
        letters_guessed.append(input_user)
        ...
        if input_user not in secret_word:
            ...
            print("干得漂亮：",get_guessed_word(secret_word,letters_guessed))
        guesses -= 1
    if is_word_guessed(secret_word, letters_guessed):
        print("--------------------------")
        print("恭喜你，你猜出来了！！！")
        print("你此次游戏得分为：",6 - guesses)
    else:
        print("--------------------------")
        print("游戏结束，您的机会已经用完。答案是：",secret_word)
