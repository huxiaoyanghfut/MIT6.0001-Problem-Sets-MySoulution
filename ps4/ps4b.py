...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for letter in ascii_lowercase:
            index = (string.ascii_lowercase.index(letter) + shift) % 26
            cipher = string.ascii_lowercase[index]
            shift_dict[letter] = cipher
        for letter in ascii_uppercase:
            index = (string.ascii_uppercase.index(letter) + shift) % 26
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        number_matched_list = []
        for s in range(26):
            message_decrypted = Message.apply_shift(self,26-s)
            message_decrypted_list = message_decrypted.split(' ') #字符串转成列表，以空格隔开每个元素   
            number_matched = 0
            for word in message_decrypted_list:
                if is_word(Message.get_valid_words(self),word):
                    number_matched += 1
            number_matched_list.append(number_matched)    
        shift_value_best = number_matched_list.index(max(number_matched_list))
        return shift_value_best,Message.apply_shift(self,26-shift_value_best)
...
#    #Example test case (CiphertextMessage)
    ...
    print(CiphertextMessage.decrypt_message(test)) 
    #test case
    print('\n')
    print("test case2:")
    test2 = Message('Jgnnq yqtnf!')
    print(test2.get_message_text())
    print(CiphertextMessage.decrypt_message(test2))
