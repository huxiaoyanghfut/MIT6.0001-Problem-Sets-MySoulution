...
def get_word_score(word, n):
    ...
    word = word.lower()
    for letter in word:
        component1 += SCRABBLE_LETTER_VALUES.get(letter, 0)
    component21 = 7*len(word) - 3*(n-len(word))
    if component21 > 1:
...
def update_hand(hand, word):
    ...
    word_dict = get_frequency_dict(word_copy)
    new_hand = {}
    for letter in hand.keys():
        if letter in word_copy:
            middle = hand.get(letter) - word_dict.get(letter)
            if middle > 0:
                new_hand[letter] = middle
            else:
                pass
        else:
...
def is_valid_word(word, hand, word_list):
    ...
    if position < 0:
        ...
        for letter in VOWELS:
            word_possible = ""
            word_lower_list[position] = letter
            for letter in word_lower_list:
                word_possible += letter
            word_possible_list.append(word_possible)
        for word_possible in word_possible_list:
            print(word_possible)
    ...
    while calculate_handlen(new_hand) > 0:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(word, hand, word_list):
                ...
                print(word, "earned", get_word_score(
                    word, calculate_handlen(hand)), "points.Total:", total_score)
    ...
    for i in range(hand_number):
        ...
        if replay_chances == 1:
            ...
            if replay.lower() == "yes" or replay.lower() == "y":
                score_hand_replay = play_hand(hand, word_list)
                replay_chances -= 1
        if score_hand_replay > score_hand:
            score_hand = score_hand_replay
        total_score_all += score_hand
    print("Total score over all hands:", total_score_all)
